...
public static class BookingLogic
{
    ...
    public static void BackfillFlightSeats()
    {
        ...
        {
            if (!FlightSeatAccess.HasAnySeatsForFlight(flight.FlightID))
            {
        ...
        {
            FlightSeatAccess.BulkCreateAllFlightSeats(toBackfill);
        }
    }
    ...
    {
        var all = BookingAccess.GetBookingsByUser(userId);
    ...
    {
        var all = BookingAccess.GetBookingsByUser(0); // or a method to get all bookings
